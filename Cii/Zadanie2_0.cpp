...
int main() {
    ...
    std::cout << std::endl;
    minRowValues(m);  // Вывод минимальных значений элементов в каждой строке матрицы
    std::cout << std::endl;

    // Вывод минимальных значений по столбцам
    minColumnValues(m);  // Вывод минимальных значений элементов в каждом столбце матрицы
    std::cout << std::endl;

    // Вывод максимальных значений по строкам
    maxRowValues(m);  // Вывод максимальных значений элементов в каждой строке матрицы
    std::cout << std::endl;

    // Вывод максимальных значений по столбцам
    maxColumnValues(m);  // Вывод максимальных значений элементов в каждом столбце матрицы
    std::cout << std::endl;

    // Вывод средних значений по строкам
    meanRowValues(m);  // Вывод среднего значения элементов в каждой строке матрицы
    std::cout << std::endl;

    // Вывод средних значений по столбцам
    meanColumnValues(m);  // Вывод среднего значения элементов в каждом столбце матрицы
    std::cout << std::endl;

    // Вывод суммы элементов в нижней треугольной части матрицы
    std::cout << "Сумма нижней треугольной части матрицы: \033[32m" << sumLowerTriangle(m) << "\033[0m" << std::endl;

    // Вывод суммы элементов в верхней треугольной части матрицы
    std::cout << "Сумма верхней треугольной части матрицы: \033[32m" << sumUpperTriangle(m) << "\033[0m" << std::endl;

    // Нахождение элемента, наиболее близкого к среднему арифметическому
    float mean = meanMatrix(m);  // Вычисление среднего арифметического всех элементов матрицы
